...
func clonePosition(pos *enginev1.Position) *enginev1.Position {
	if pos == nil {
		return nil
	}
...
func setAttackOrderLastKnown(order *enginev1.AttackOrder, target *enginev1.Unit, simSeconds float64) {
	if order == nil || target == nil {
		return
	}
	order.LastKnownTargetPosition = clonePosition(target.GetPosition())
...
func (a *App) AppendMoveWaypoint(unitID string, lat, lon float64) BridgeResult {
	...
	for _, u := range a.currentScenario.Units {
		...
		if isMaritimeDomain(defs[u.DefinitionId].Domain) {
			...
			routePoints = rerouted
		}
		for idx, routePoint := range routePoints {
			legStartLat, legStartLon := startLat, startLon
			if idx > 0 {
...
func (a *App) SetUnitAttackOrder(unitID string, orderType int32, targetUnitID string, desiredEffect int32, pkillThreshold float32) BridgeResult {
	...
	for _, u := range a.currentScenario.Units {
		...
		weaponID := ""
		if decision := sim.EvaluateCurrentEngagement(
			u,
			target,
			...
			a.getSimSeconds(),
			isPreplannedFixedTarget(targetDef),
		); decision.WeaponID != "" {
		...
		setAttackOrderLastKnown(u.AttackOrder, target, a.getSimSeconds())
		if current := u.GetMoveOrder(); current == nil || len(current.GetWaypoints()) == 0 || current.GetAutoGenerated() {
			if waypoint := sim.ComputeAttackWaypointForOrder(u, target, a.buildDefs(), a.buildWeaponCatalog()); waypoint != nil {
				...
				if isMaritimeDomain(defs[extractRecordID(u.DefinitionId)].Domain) {
					...
					if !ok {
						return failMsg("naval route to attack position crosses land")
					}
