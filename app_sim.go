...
func (a *App) MoveUnit(unitID string, lat, lon float64) BridgeResult {
	...
	for _, u := range a.currentScenario.Units {
		...
		newPos := geo.ProtoPosition(oldPos.GetLat(), oldPos.GetLon(), travelAlt, heading, cruiseSpeed)
		newOrder := &enginev1.MoveOrder{AutoGenerated: false}
		for _, point := range route.Points {
			newOrder.Waypoints = append(newOrder.Waypoints, geo.ProtoWaypoint(point.Lat, point.Lon, travelAlt))
		}
		u.Position = newPos
...
func (a *App) CancelMoveOrder(unitID string) BridgeResult {
	...
	for _, u := range a.currentScenario.Units {
		...
		pos := u.GetPosition()
		stoppedPos := geo.ProtoPosition(pos.GetLat(), pos.GetLon(), pos.GetAltMsl(), pos.GetHeading(), 0)
...
func (a *App) ReturnUnitToBase(unitID string) BridgeResult {
	...
	for _, routePoint := range route.Points {
		...
		moveOrder.Waypoints = append(moveOrder.Waypoints, geo.ProtoWaypoint(routePoint.Lat, routePoint.Lon, alt))
	}
...
func (a *App) AppendMoveWaypoint(unitID string, lat, lon float64) BridgeResult {
	...
	for _, u := range a.currentScenario.Units {
		...
		for _, routePoint := range route.Points {
			newOrder.Waypoints = append(newOrder.Waypoints, geo.ProtoWaypoint(routePoint.Lat, routePoint.Lon, travelAlt))
		}
		u.MoveOrder = newOrder
		...
		if !hadWaypoints && len(newOrder.Waypoints) > 0 {
			...
			newPos = geo.ProtoPosition(pos.GetLat(), pos.GetLon(), travelAlt, heading, cruiseSpeed)
			u.Position = newPos
		}
