...
func (a *App) buildDefs() map[string]sim.DefStats {
	...
	for _, row := range rows {
		...
		defs[id] = sim.DefStats{
			...
			EmbarkedFixedWingCapacity:   int(toFloat64(row["embarked_fixed_wing_capacity"])),
			EmbarkedRotaryWingCapacity:  int(toFloat64(row["embarked_rotary_wing_capacity"])),
			EmbarkedUAVCapacity:         int(toFloat64(row["embarked_uav_capacity"])),
